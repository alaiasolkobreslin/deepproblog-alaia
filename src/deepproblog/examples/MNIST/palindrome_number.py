...
def palindrome_number(n: int, dataset: str, seed=None):
    return MNISTOperator(
        dataset_name=dataset,
        function_name="palindrome_number",
        operator=lambda x: is_palindrome(x),
        size=n,
        arity=3,
        seed=seed,
    )

parser = ArgumentParser("palindrome_number")
...
name = "palindrome_number{}_{}_{}".format(method, N, args.seed)

train_set = palindrome_number(N, "train")
test_set = palindrome_number(N, "test")
...
model = Model("models/palindrome_number.pl", [net])
if method == "exact":
