...
def jump_game(n: int, dataset: str, seed=None):
    return MNISTOperator(
        dataset_name=dataset,
        function_name="main",
        operator=lambda x: 1 if execute_jump_game(x) else 0,
        size=n,
        arity=5,
        seed=seed,
    )

parser = ArgumentParser("jump_game")
...
name = "jump_game{}_{}_{}".format(method, N, args.seed)

train_set = jump_game(N, "train")
test_set = jump_game(N, "test")
...
model = Model("models/jump_game.pl", [net])
if method == "exact":
