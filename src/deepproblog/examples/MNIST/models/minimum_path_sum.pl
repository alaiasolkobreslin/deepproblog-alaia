...
replace([_|T], 0, E, [E|T]) :- !.
replace([H|T], N, E, [H|TE]) :- N1 is N - 1, replace(T, N1, E, TE). 

replace_2d([H|T], (0, C), E, [NH|T]) :- replace(H, C, E, NH), !.
replace_2d([H|T], (R, C), E, [H|NT]) :- R1 is R - 1, replace_2d(T, (R1, C), E, NT).

nth_2d(X, (R, C), E) :- nth0(R, X, L), nth0(C, L, E).

create(0, []) :- !.
create(N, [0|T]) :- N1 is N - 1, create(N1, T).

create_2d(0, _, []) :- !.
create_2d(M, N, [H|T]) :- create(N, H), M1 is M - 1, create_2d(M1, N, T). 

update(M, V, (0, 0), NM) :- !, nth_2d(V, (0,0), E), replace_2d(M, (0,0), E, NM).
update(M, V, (0, C), NM) :- !, C1 is C - 1, 
                            nth_2d(V, (0,C), N1), nth_2d(M, (0, C1), N2),
                            N is N1 + N2,
                            replace_2d(M, (0,C), N, NM).
update(M, V, (R, 0), NM) :- !, R1 is R - 1, 
                            nth_2d(V, (R,0), N1), nth_2d(M, (R1, 0), N2),
                            N is N1 + N2,
                            replace_2d(M, (R,0), N, NM).
update(M, V, (R, C), NM) :- C1 is C - 1, R1 is R - 1,
                            nth_2d(V, (R,C), N1),
...
next_dir(M, (R, _), (R1, 0)) :- R1 is R + 1, nth_2d(M, (R1, 0), _).

path_sum(M, V, C, NM) :- update(M, V, C, R), next_dir(M, C, NC), !,
                         path_sum(R, V, NC, NM).
path_sum(M, V, C, NM) :- update(M, V, C, NM).        
...
minimum_path_sum(X, N) :- num_list_2d(X, M), min_path(M, N).