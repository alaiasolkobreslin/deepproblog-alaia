...
replace_2d([H|T], (R, C), E, [H|NT]) :- R1 is R - 1, 
...
next_dir(M, D, C, C1, D1) :- ops(D, C, C1, _, D1, _),
                             nth_2d(M, C1, 0), !.
next_dir(_, D, C, C2, D2) :- ops(D, C, _, C2, _, D2).

spiral(M, _, [], M).
spiral(M, _, C, [H], NM) :- !, replace_2d(M, C, H, NM).
spiral(M, D, C, [H|T], R) :- replace_2d(M, C, H, NM), next_dir(M, D, C, NC, ND),
                             spiral(NM, ND, NC, T, R).

spiral_matrix_ii(X, M) :- number(X, N),
